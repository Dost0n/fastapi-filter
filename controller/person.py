...
@router.patch('update/{id}', response_model=ResponseSchema, response_model_exclude_none=True)
async def update_person(person_id:int = Path(..., alias='id'), *, update_form:PersonCreate):
    await PersonRepository.update(person_id, update_form)
    return ResponseSchema(detail="Successfully update data")


@router.delete("delete/{id}", response_model=ResponseSchema, response_model_exclude_none=True)
async def delete_person(person_id:int = Path(..., alias="id"),):
